...

    {
        ...
        public int Update(Client item)
        {
            using (SqliteConnection connection = new SqliteConnection(connectionString))
            {
                ...
                //Прописываем в команду SQL-запрос на добавление данных
                SqliteCommand command = connection.CreateCommand();
                command.CommandText = "UPDATE Clients SET Document = @Document, SurName = @SurName, " +
                    "FirstName = @FirstName, Patronymic = @Patronymic, Birthday " +
                    "= @Birthday WHERE ClientId=@ClientId";
